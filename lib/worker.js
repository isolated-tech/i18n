...
self.addEventListener('message', async event => {
  ...
  let obj
  let textBeforeFirstBrace
  const fileType = event.data.fileType
  const response = parseDataBasedOnFileType(event.data.text, fileType)
  if (fileType === 'js') {
    obj = response.obj
    textBeforeFirstBrace = response.textBeforeFirstBrace
  } else {
    obj = response
  }
...
const parseDataBasedOnFileType = (text, fileType) => {
  switch (fileType) {
    ...
    case 'js':
      try {
        const textBeforeFirstBrace = text.substring(0, text.indexOf('{'))
